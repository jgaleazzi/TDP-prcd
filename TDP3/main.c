...
int main(int agrc, char**argv){
  ...
  if(nb_proc==1){
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, size, size,
		size, 1.0, A,
		size, B, size,
		0, C, size);
   ...
   int displs[nb_proc];
   double *local_A= calloc(block_size*block_size,sizeof(double));
   double *block_B=calloc(block_size*block_size,sizeof(double));
   ...
   MPI_Type_extent(MATRIX_BLOC, &extent);
   ...
   double *bloc_C=calloc(block_size*block_size,sizeof(double));
   double *current_A=calloc(block_size*block_size,sizeof(double));
